...
type ICoffeeCard = Omit<CoffeeItem, 'id'>

export const CoffeeCard: React.FC<ICoffeeCard> = ({ name, image, price, rating, votes, available, popular }) => {
  return (
    <div className={styles.card}>
      <div className={styles.image}>
        <img
          ...
          src={image}
          alt={`Tasty cup of '${name}'`}
          loading='lazy'
          data-popular={popular}
          width={260}
          height={160}
        />
        {popular && <span className={styles.image__tag}>popular</span>}
      </div>
      <div className={styles.card__detail}>
        <h3 className={styles.detail__title}>{name}</h3>
        <div className={styles.detail__price}>{price}</div>
        <div className={styles.detail__rating}>
          {rating ? (
            <>
              <div className={styles['rating__icon--fill']}>
              ...
              <span className={styles.rating__votes}>({votes}&nbsp;votes)</span>
            </>
          ) : (
            <>
              ...
              <span className={styles.rating__votes}>No ratings</span>
            </>
          )}
        </div>
        {!available && <div className={styles.detail__available}>Sold out</div>}
      </div>
    </div>
  )
}
